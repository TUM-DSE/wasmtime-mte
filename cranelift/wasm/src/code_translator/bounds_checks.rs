    ...
    Env: FuncEnvironment + ?Sized,
{
    ...
    if env.mte_bounds_checks() {
        ...
        let index_addr = if env.mem_safety() {
            // we need to mask out the lowest tag bit, as that is used for bounds check.
            builder.ins().band_imm(index_addr, 0xFEFF_FFFF_FFFF_FFFFu64 as i64)
        } else {
            index_addr
        };
