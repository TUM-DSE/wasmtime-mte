    ...
    Env: FuncEnvironment + ?Sized,
{
    ...
    let index = if env.mte_mem_safety() {
        builder
            .ins()
    ...
    if env.mte_bounds_checks() {
        ...
        // If mte bounds checks and memory safety are enabled, tags generated by irg will be in the tag bits 1..=3,
        // while the tag in the heap base will be in the tag bit 0.
        // To prevent attackers from hand-crafting indices that will overflow the lowest tag bit, we set it again here.
        if env.mte_mem_safety() {
            address = builder.ins().bor_imm(address, 1 << 56);
        }

        return Ok(Reachable(address));
    }
